...
RSpec.describe NotesController, :type => :controller do
  ...
  describe '#show' do
    before do
      @note = create(:note, text: 'my cute note')
    end
    it 'displays a single note' do
      get :show, id: @note.id
      expect(response).to be_success
      expect(assigns(:note).text).to eq 'my cute note'
  ...
  describe '#create' do
    context 'when saving a proper record' do
      it 'creates a new note saves it to the db' do
        expect {
          post :create, note: {text: 'a some new note'}
  ...
  describe '#update' do
    ...
    context 'when the update failed' do
      it 'fails to update the text from "this" to "that" to nil and renders the edit template' do
